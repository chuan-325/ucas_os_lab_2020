...
void set_pcb(pid_t pid, pcb_t *pcb, task_info_t *task_info, uint64_t para) {
  // basic info
  pcb->pid = pid;
  pcb->type = task_info->type;
  pcb->prior = task_info->prior;
  pcb->state =
      (task_info->type == USER_PROCESS || task_info->type == USER_THREAD)
          ? STATE_USER
          : STATE_KERNEL;
  ...
  // para
  if (para != 0) {
    pcb->user_context.regs[4] = para;
    pcb->kernel_context.regs[4] = para;
  }
...
int do_spawn(task_info_t *task, uint64_t para) {
  int i;
  i = alloc_pcb();
  if (i == -1)
    return -1;
  set_pcb(++process_id, &pcb[i], task, para);
...
void do_process_show() {
  ...
  for (i = 0; i < NUM_MAX_TASK; i++) // show running
  {
    ...
    case TASK_RUNNING:
      vt100_move_cursor(pcb[0].cursor_x, ++pcb[0].cursor_y);
      printk("[%d] PID = %d STATUS = RUNNING", num_ps++, pcb[i].pid);
      break;
    case TASK_READY:
      vt100_move_cursor(pcb[0].cursor_x, ++pcb[0].cursor_y);
      printk("[%d] PID = %d STATUS = READY", num_ps++, pcb[i].pid);
      break;
    case TASK_BLOCKED:
      vt100_move_cursor(pcb[0].cursor_x, ++pcb[0].cursor_y);
      printk("[%d] PID = %d STATUS = BLOCKED", num_ps++, pcb[i].pid);
      break;
    default:
      break;
    }
